      ...
      try {
        ...
        if ((dom === "my-schedules" || dom === "schedule" || dom === "schedule-dashboard") && sid) {
          ...
          if (sc.rows[0]) {
            ...
            // per-faculty weekly timetable (so 'X's schedule' can be answered)
            const ftt = await db.query("SELECT faculty, day, time_start, time_end, subject, class_name, location FROM public.weekly_schedule WHERE schedule_id=$1 AND faculty!='_locations_' AND (type IS NULL OR type='') ORDER BY faculty, array_position(ARRAY['Mon','Tue','Wed','Thu','Fri','Sat','Sun'], day), time_start", [sid]);
            const byFac = {};
            ...
            const facSched = Object.keys(byFac).map(fn => "  • " + fn + ": " + byFac[fn].join("; ")).join("\n");
            let weeks = "";
