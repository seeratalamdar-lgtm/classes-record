...
export default function EntryScreen() {
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: async (_, vars) => {
      ...
      if (vars.Type === "Makeup" && user) {
        ...
        Alert.alert("Notice", String("✅ Entry saved successfully!"));
      }
    },
    onError: () => Alert.alert("Notice", String("❌ Failed to save entry. Please try again.")),
  });
  ...
  async function handleBulkImport() {
    if (!bulkDate) { Alert.alert("Notice", String("Please select a date.")); return; }
    ...
    } catch(e: any) { Alert.alert("Notice", String("Import failed: " + e.message)); }
  ...
  async function handleSaveMakeupFromEntry() {
    ...
    try {
      ...
      const data=await res.json();
      if(data.success){ Alert.alert("Notice", String("✅ Makeup entry saved for "+addMakeupDate+"!")); setAddingMakeupEntry(null); setAddMakeupDate(""); handleViewEntries(); }
      else if(data.clash){ Alert.alert("Notice", String("Clash Detected: "+data.message)); }
      else { Alert.alert("Notice", String("Save failed: "+(data.error||data.message||"Unknown"))); }
    } catch(ex:any){ Alert.alert("Notice", String("Error: "+ex.message)); }
  ...
  async function handleAddSingleEntry() {
    ...
    try {
      ...
      else { Alert.alert("Notice", String("Save failed: "+(data.error||"Unknown error"))); }
    } catch(e:any){ Alert.alert("Notice", String("Error: "+e.message)); }
    setAddSaving(false);
  }
  ...
  async function handleDeleteEntry(id: number) {
    ...
    try {
      ...
      else { Alert.alert("Notice", String("Delete failed")); }
    } catch(e) { Alert.alert("Notice", String("Error deleting entry")); }
  }
  ...
  function handleEntriesExportCsv() {
    if (!allEntries.length && !schedule.length) { Alert.alert("Notice", String("No entries to export.")); return; }
  ...
  async function handleEntriesOverride(uri:string, name:string, mimeType:string, file?:File) {
    ...
    try {
      ...
      if (result.success) { setShowEntriesOverride(false); Alert.alert("Notice", String("Override complete! "+result.imported+" entries updated.")); handleViewEntries(); }
      else Alert.alert("Notice", String("Override failed: "+(result.error||"Unknown error")));
    } catch(e:any) { Alert.alert("Notice", String("Override failed: "+e.message)); }
        ...
        <View style={{position:"absolute",top:0,left:0,right:0,bottom:0,backgroundColor:"rgba(0,0,0,0.5)",justifyContent:"flex-end",zIndex:9999}}>
          <View style={{backgroundColor:"#fff",borderTopLeftRadius:20,borderTopRightRadius:20,padding:24,maxHeight:"80%"}}>
            <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
              ...
              <TouchableOpacity onPress={() => { setShowNotify(false); Alert.alert("Notice", String("✅ Makeup entry saved successfully!")); }}>
            ...
            <TouchableOpacity
              ...
              onPress={() => { setShowNotify(false); Alert.alert("Notice", String("✅ Makeup entry saved successfully!")); }}
            >
